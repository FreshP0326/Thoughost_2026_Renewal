...
function SocialIcon({ iconKey, label, dark = false }: { iconKey: string; label: string; dark?: boolean }) {
  if (["x", "bandcamp", "soundcloud", "bilibili", "dizzylab"].includes(iconKey)) {
    const iconSize = iconKey === "soundcloud" ? "var(--header-soundcloud-icon-size)" : "var(--header-social-icon-size)";
...
export function SocialLinks({
  ...
  return (
    <div className="flex items-center gap-[var(--header-social-gap)]">
      {links.map((item) => (
        <Link
          key={item.platform}
          href={item.url}
